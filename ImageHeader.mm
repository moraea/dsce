...
- (NSArray<NSString*>*)dylibPathsReexportOnly:(BOOL)reexportOnly {
    ...
    [self forEachCommand:^(struct load_command* command) {
        if (command->cmd == LC_LOAD_DYLIB || command->cmd == LC_LOAD_WEAK_DYLIB || command->cmd == LC_LOAD_UPWARD_DYLIB ||
...
- (void)dumpSegments {
    [self forEachSegmentCommand:^(struct segment_command_64* seg) {
        trace(@"segment %s address %lx offset %lx memory size %lx file size %lx", seg->segname, seg->vmaddr, seg->fileoff, seg->vmsize,
              seg->filesize);
    }];
}
